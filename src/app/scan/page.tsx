...
export default function ScannerPage() {
  ...
  const [regNo, setRegNo] = useState('') // New state for Registration No
  ...
  // 1. LOGIN HANDLER
  const handleLogin = () => {
    // Keep your PIN as 2026
    if (pin === '2026') { 
        if (!adminName.trim()) return alert("Please enter your name first.")
        if (!regNo.trim()) return alert("Please enter your registration number.")
        setAuthorized(true)
    } else {
        alert('Wrong PIN')
    }
  ...
  const processScan = async () => {
    ...
    await supabase.from('scan_history').insert([{
        ...
        scanned_by_reg_no: regNo, // Logging the Reg No to history
        scanned_at: istNow
    }])
  ...
  if (!authorized) return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-900 text-white p-6 relative">
          ...
          <div className="w-full max-w-xs space-y-6">
            ...
            {/* NEW: REGISTRATION NO BOX */}
            <div>
              <label className="block text-gray-300 mb-2 text-sm font-bold uppercase">Registration No</label>
              <input 
                className="w-full p-4 rounded-lg text-black font-bold bg-white outline-none focus:ring-4 ring-blue-500" 
                placeholder="Ex: 2026CS101" 
                value={regNo} 
  ...
  // --- UI: SCANNER SCREEN (Same as before) ---
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center p-4 relative">
      <div className="w-full flex justify-between mb-4 items-center max-w-sm">
        <span className="text-gray-400 text-sm">Operator: <span className="text-white font-bold">{adminName} ({regNo})</span></span>
      ...
      {showVenueModal && detectedUser && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-50 p-4">
            <div className="bg-white text-black p-6 rounded-xl w-full max-w-xs text-center shadow-2xl relative">
                ...
                <button onClick={processScan} className={`w-full text-white font-bold py-4 rounded-lg text-lg shadow-lg transition-all mb-3 ${detectedUser.status === 'checked_in' ? 'bg-red-600 hover:bg-red-500' : 'bg-green-600 hover:bg-green-500'}`}>
      ...
      <div className={`mt-6 w-full max-w-sm p-4 rounded-xl text-center font-bold text-lg border-2 ${statusMessage.includes('SUCCESS') ? 'bg-green-900/90 border-green-500 text-green-100' : statusMessage.includes('Error') || statusMessage.includes('Invalid') ? 'bg-red-900/90 border-red-500 text-red-100' : 'bg-gray-800 border-gray-700 text-gray-400'}`}>
